...
The three pages declare `chrome: 'bare'`, so an app shell that honours that
...
| `sendPasswordResetEmail` | to enable forgot-password | the obvious fallback — log it — writes a working credential into the app's log aggregator. Absent, the endpoint refuses rather than minting a token nobody receives |
...
`full` · `pwd_change` · `mfa`. A `pwd_change` token resolves to **no permission
...
- **Lockout and IP limits are both required.** Lockout alone lets an attacker
  ...
  across accounts. Neither is sufficient.
...
| `AuthMfaFactor` | **schema only** — no endpoint enrols or verifies one yet |
| `AuthRecoveryCode` | **schema only** |

The `mfa` token scope exists alongside those last two and nothing issues it yet.
Treat all four as reserved shape, not as working features.
