import { getSessionToken, getViewer } from '@kwtech/module-auth/next';
import { getPermissionContext } from '@kwtech/module-permissions/next';
...
export async function AppShell({ title, children }: { title: string; children: ReactNode }) {
   ...
   * server (PLAN §9).
   */
  const [viewer, token, cookieStore] = await Promise.all([getViewer(), getSessionToken(), cookies()]);
  const permissions = await getPermissionContext({ token });
